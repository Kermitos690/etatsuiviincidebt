...
const primaryNavItems = [
  ...
  { to: '/analysis-pipeline', icon: Sparkles, label: 'Pipeline' },
  { to: '/incidents', icon: AlertTriangle, label: 'Incidents' },
];

const moreNavItems = [
  { to: '/emails-analyzed', label: 'Emails Analysés' },
  { to: '/attachments', label: 'Pièces Jointes' },
  { to: '/violations', label: 'Violations' },
  { to: '/journal', label: 'Journal' },
  { to: '/incidents-timeline', label: 'Timeline' },
    ...
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-glass backdrop-blur-glass border-t border-glass safe-area-bottom">
      <div className="flex items-center justify-around h-16 px-2">
        ...
        <Sheet open={open} onOpenChange={setOpen}>
          ...
          <SheetContent 
            side="bottom" 
            className="bg-glass backdrop-blur-glass border-t border-glass rounded-t-3xl max-h-[70vh] overflow-y-auto"
          >
            <div className="py-4">
              <div className="w-12 h-1 bg-muted-foreground/30 rounded-full mx-auto mb-6" />
              <h3 className="text-lg font-semibold text-center mb-4 gradient-text">Plus d'options</h3>
